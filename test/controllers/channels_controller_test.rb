...
class ChannelsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "create_failure_duplicate" do
    post "/api/channels", params: { channel: { name: "Test text channel3", category: "text", description: "This is a test channel", guild_id: "600a75db-1486-4719-bbb8-776e4cbebde1"} }

    assert_response(:unprocessable_entity)
    response = JSON.parse(@response.body)
    assert_equal "The channel name has already be taken in this guild", response["error"]
  end 
  ...
  test "show_failure_channelNotFound" do
    ...
    response = JSON.parse(@response.body)
    assert_equal "The channel was not found", response["error"]
  end
  ...
  test "update_failure_channelNotFound" do
  ...
  test "update_failure_guildNotFound" do
    patch "/api/channels/701a75db-1486-4719-bbb8-776e4cbebde1", params: { name: "TTC3", category: "text", description: "just a test", guild_id: "Not gonna be found" }
    assert_response(:not_found)
